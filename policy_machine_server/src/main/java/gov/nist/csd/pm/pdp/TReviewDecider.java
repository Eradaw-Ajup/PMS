...
public class TReviewDecider implements Decider {

	    private GraphMachine graph;
	     ...
	     */
	    public TReviewDecider(GraphMachine graph) {
	        if (graph == null) {
	    ...
	    public boolean hasPermissions(long userID, long targetID, String... perms) throws PMException {
	        ...
	        if (permsToCheck.contains(Operations.OBJECT_ACCESS)) {
	            return !permissions.isEmpty();
	        }
	        else if (permissions.contains(Operations.OBJECT_ACCESS)) {
	            return true;
	        }
	    ...
	    private HashSet<String> resolvePermissions(Map<Long, Set<String>> pcMap) {
	        ...
	        for (long pc : pcMap.keySet()) {
	            ...
	            if(first) {
	                ...
	                if (perms.contains(Operations.OBJECT_ACCESS)) {
	                    perms.remove(Operations.OBJECT_ACCESS);
	                    perms.addAll(ops);
	                } else {
	                    ...
	                    if (ops.isEmpty()) {
	                        perms.clear();
	                        break;
	                    } else if (!ops.contains(Operations.OBJECT_ACCESS)) {
	                        perms.retainAll(ops);
	                    }
	                }
	            }
	        ...
	        if (perms.contains(Operations.OBJECT_ACCESS)) {
	            perms.clear();
	            perms.add(Operations.OBJECT_ACCESS);
	        }
	    ...
	    private synchronized long createVNode(Map<Long, Set<String>> dc) throws PMException {
	        /*
	    	MNode vNode = graph.createNode(new Random().nextLong(), "VNODE", NodeType.OA, null, StoreType.N);
	        for (long nodeID : dc.keySet()) {
	            graph.assign(nodeID, vNode.getID(), true, StoreType.N);
	        }
	        return vNode.getID();
	        */
	    	return 0;
