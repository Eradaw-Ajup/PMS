...
/*
	...
	void setup() throws PMException, IOException {
		...
		GraphService service = new GraphService(SessionsService.getUserNodeID("1222"), null);

		// create a node for each type
		testPC = service.createNode("pc1", PC, NodeUtils.toProperties("namespace", testID), NodeStoreType.SN);
		testOA = service.createNode("oa1", NodeType.OA, NodeUtils.toProperties("namespace", testID), NodeStoreType.SN);
		testO = PAP.getPAP().getGraphPAP().createNode(new Random().nextLong(), "testO", O,
				NodeUtils.toProperties("namespace", testID));
		testUA = service.createNode("ua1", UA, NodeUtils.toProperties("namespace", testID), NodeStoreType.SN);
		testUser = PAP.getPAP().getGraphPAP().createNode(new Random().nextLong(), "u1", U,
				NodeUtils.toProperties("namespace", testID));
		...
		PAP.getPAP().getGraphPAP().associate(testUA.getID(), testOA.getID(), new HashSet<>(Arrays.asList(DISASSOCIATE,
				ASSOCIATE, ASSIGN, ASSIGN_TO, UPDATE_NODE, DELETE_NODE, DEASSIGN_FROM, DEASSIGN)));
	}
	...
	void TestCreateNode() throws PMException {
		...
		final GraphService service1 = new GraphService(superUser.getID(), null);
		assertAll(() -> assertThrows(IllegalArgumentException.class, () -> service1.createNode(null, null, null, null)),
				() -> assertThrows(IllegalArgumentException.class, () -> service1.createNode(null, OA, null, null)),
				() -> assertThrows(IllegalArgumentException.class,
						() -> service1.createNode("test_node", null, null, null)),
				() -> assertThrows(PMException.class,
						() -> service1.createNode( "oa1", NodeType.OA, NodeUtils.toProperties("namespace", testID), null)));

		// create a policy class
		// check that an unauthorized user cannot create a policy class
		final GraphService service2 = new GraphService(testUser.getID(), null);
		assertThrows(PMException.class,
				() -> service2.createNode("test_pc", PC, NodeUtils.toProperties("key1", "value1"), NodeStoreType.SN));

		// authorized user - check all pc elements were created
		final GraphService service3 = new GraphService(superUser.getID(), null);
		Node testPC = service3.createNode("test_pc", PC, NodeUtils.toProperties("namespace", testID), NodeStoreType.SN);
		...
		final GraphService service4 = new GraphService(testUser.getID(), null);
		assertThrows(PMException.class,
				() -> service4.createNode("newNode", OA, NodeUtils.toProperties("namespace", testID), NodeStoreType.SN));

		// user can assign to parent
		final GraphService service5 = new GraphService(superUser.getID(), null);
		Node newNode = service5.createNode("newNode", OA, NodeUtils.toProperties("namespace", testID), null);
	...
	void TestUpdateNode() throws PMException {
		...
		service1.updateNode(testOA.getID(), null, NodeUtils.toProperties("test", "updated namespace"), NodeStoreType.SN);

		assertAll(
				() -> assertThrows(IllegalArgumentException.class,
						() -> service1.updateNode(0, "update_node_no_id", null, null)),
				() -> assertThrows(PMException.class, () -> service1.updateNode(new Random().nextLong(), "updated_name",
						NodeUtils.toProperties("updatedKey", "updatedValue"), null)));
		// user can update node
		service1.updateNode(testOA.getID(), "updated_name",
				NodeUtils.toProperties("updatedKey", "updatedValue", "namespace", testID), NodeStoreType.SN);

		Node node = service1.getNode(testOA.getID());
		assertEquals(node.getName(), "updated_name", "node name does not match");
		assertEquals(node.getProperties(), NodeUtils.toProperties("updatedKey", "updatedValue", "namespace", testID),
				"properties do not match");

		// user cannot update node
//		GraphService service2 = new GraphService(testUser.getID(), null);
//		assertThrows(PMException.class,
//				() -> service2.updateNode(PAP.getPAP().getSuperO().getID(), "updated super name", null, null));
	}

	@Test
	void TestDeleteNode() throws PMException {
		// test user tries to delete super o
		GraphService service = new GraphService(testUser.getID(), null);
		assertThrows(PMException.class, () -> service.deleteNode(testOA.getID(), NodeStoreType.SN));

		GraphService service1 = new GraphService(superUser.getID(), null);
		// super user deletes test object
		service1.deleteNode(testO.getID(), NodeStoreType.SN);
		assertFalse(PAP.getPAP().getGraphPAP().exists(testO.getID()));

		// super user deletes test pc
		service1.deleteNode(testPC.getID(), NodeStoreType.SN);
		assertFalse(PAP.getPAP().getGraphPAP().exists(testPC.getID()));
	}

	@Test
	void TestExists() throws PMException {
		GraphService service = new GraphService(superUser.getID(), null);
		assertTrue(service.exists(testPC.getID()));
		assertFalse(service.exists(new Random().nextLong()));
	}


	 * @Test void TestGetNodes() throws PMException { GraphService service = new
	 * GraphService(superUser.getID(), 0); HashSet<Node> nodes = service.getNodes();
	 * assertTrue(nodes.size() >= 7,
	 * "the super user should have access to at least 7 nodes"); GraphService
	 * service1 = new GraphService(testUser.getID(), 0); HashSet<Node> nodes1 =
	 * service1.getNodes(); assertEquals(2, nodes1.size(),
	 * "the test user should have access to 2 nodes"); }
	...
	void TestAssign() throws PMException {
		...
		GraphService service1 = new GraphService(testUser.getID(), null);
		//assertThrows(PMException.class, () -> service1.assign(testO.getID(), PAP.getPAP().getSuperOA().getID()));
	}
	...
	void TestDeassign() throws PMException {
		...
		GraphService service1 = new GraphService(testUser.getID(), null);
//		assertThrows(PMException.class,
//				() -> service1.deassign(PAP.getPAP().getSuperO().getID(), PAP.getPAP().getSuperOA().getID()));
	}
...
//		assertTrue(PAP.getPAP().getGraphPAP().getSourceAssociations(PAP.getPAP().getSuperUA2().getID())
//				.containsKey(testOA.getID()));
//		assertTrue(PAP.getPAP().getGraphPAP().getSourceAssociations(PAP.getPAP().getSuperUA2().getID())
//				.get(testOA.getID()).contains("test"));
//		service.associate(PAP.getPAP().getSuperUA2().getID(), testOA.getID(), new HashSet<>(Arrays.asList("test123")));
//		assertTrue(PAP.getPAP().getGraphPAP().getSourceAssociations(PAP.getPAP().getSuperUA2().getID())
//				.get(testOA.getID()).contains("test123"));
//
//		GraphService service1 = new GraphService(testUser.getID(), null);
//		assertThrows(PMException.class, () -> service1.associate(PAP.getPAP().getSuperUA2().getID(), testOA.getID(),
...
//		assertFalse(PAP.getPAP().getGraphPAP().getSourceAssociations(PAP.getPAP().getSuperUA2().getID())
